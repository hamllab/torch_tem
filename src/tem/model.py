...
The referenced paper is the bioRxiv publication at
https://www.biorxiv.org/content/10.1101/770495v2
...
class Model(torch.nn.Module):
    def __init__(self, params):
        # First call super class init function to set up torch.nn.Module style model
        # and inherit its functionality
    ...
    def forward(self, walk, prev_iter=None, prev_M=None):
        ...
        # for which some parameters need to be reset.
        steps = self.init_walks(prev_iter)
        # Forward pass: perform a TEM iteration for each set of
        # [place, observation, action], and produce inferred and generated variables
        # for each step.
        for g, x, a in walk:
            # If there is no previous iteration at all: all walks are new; initialise
            # a whole new iteration object
            if steps is None:
                # Use an Iteration object to set initial values before any real
                # iterations, initialising M, x_inf as zero. Set actions to None blank
    ...
    def iteration(self, x, locations, a_prev, M_prev, x_prev, g_prev):
        # First, do the transition step, as it will be necessary for both the
        # inference and generative part of the model
        gt_gen, gt_inf = self.gen_g(a_prev, g_prev, locations)
        # Run inference model: infer grounded location p_inf (hippocampus), abstract
        # location g_inf (entorhinal). Also keep filtered sensory observation (x_inf),
        ...
        # purposes, it will generate from *inferred* variables instead of *generated*
        # variables (as it would when used for generation)
        ...
        if self.hyper["use_p_inf"]:
            # Inference memory is identical to generative memory if using common
            # memory, and updated separately if not
            M.append(
                M[0]
    ...
    def generative(self, M_prev, p_inf, g_inf, g_gen):
        ...
        # frequency. Also keep non-softmaxed logits which are used in the loss later
        ...
        # Retrieve grounded location from memory by pattern completion on abstract
    ...
    def loss(self, g_gen, p_gen, x_logits, x, g_inf, p_inf, p_inf_x, M_prev):
        ...
        # to reweight contributions later
        # L_p_gen is squared error loss between inferred grounded location and grounded
        # location retrieved from inferred abstract location
        ...
        # location retrieved from sensory experience
        L_p_x = (
        ...
        # predictions. First get true labels from sensory experience
    ...
    def init_trainable(self):
        ...
        # comes first, low frequency comes last. Learn inverse sigmoid instead of scale
        # factor directly, so domain of alpha is -inf, inf
        ...
        # like a prior on g. Initialise with truncated normal
        ...
        # environment; standard deviation of the prior on g. Initialise with truncated
        # normal
        ...
        # learn about similarities between actions). Size is given by grid connections
        self.MLP_D_a = MLP(
        ...
        # the current abstract location to predict the next abstract location
        ...
        # MLP for standard deviation of grounded location from retrieved memory sample
        ...
        # obtained by summing grounded location over sensory preferences in inference
        # model
        ...
        # (generated observation error and inferred abstract location vector norm) of
        # memory quality
        ...
        # MLP to generate mean of abstract location directly from shiny object
        # presence. Outputs to object vector cell modules if they're separated, else to
        # all abstract location modules
        ...
        # object presence. Outputs to object vector cell modules if they're separated,
    ...
    def init_iteration(self, g, x, a, M):
        ...
        # useful when doing analysis on the network with different batch sizes compared
        # to training
        ...
        if M is None:
            ...
            if self.hyper["use_p_inf"]:
                ...
                # connectivity, and same memory vectors. Else, create a new empty
                # memory list for inference network
                M.append(
                    M[0]
                    if self.hyper["common_memory"]
                    else torch.zeros(
        ...
        # Initialise previous sensory experience with zeros, as there is no data yet
    ...
    def init_walks(self, prev_iter):
        ...
        # actually provided - if it wasn't, all parameters will be reset when creating
        # a fresh Iteration object in init_iteration
        if prev_iter is not None:
            # The supplied previous iteration might have new walks starting, with empty
            # actions. For these walks some parameters need to be reset
    ...
    def gen_g(self, a_prev, g_prev, locations):
        ...
        # weights specific to action taken for each frequency module
        ...
        # location shouldn't have access to the action direction in the generative
        # model
        shiny_envs = [location["shiny"] is not None for location in locations]
        # If there are any shiny environments, the abstract locations for the
        # generative model will need to be re-calculated without providing actions for
        # those
        g_gen = (
    ...
    def gen_p(self, g, M_prev):
        ...
        # right dimensions (these are grid cells, we need place cells). We need g_
        # instead
        g_ = self.g2g_(g)
    ...
    def gen_x(self, p):
        ...
        # If you actually want to sample observation, you need a reparameterisation
        # trick for categorical distributions. Sampling would be the correct way to do
        # this, since observations are discrete, and it's also what the TEM paper says.
        # However, it looks like you could also get away with using categorical
        # distribution directly as an approximation of the one-hot observations
        if self.hyper["do_sample"]:
            x, logits = self.f_x(p)
    ...
    def inf_p(self, x_, g_):
        # Infer grounded location from sensory experience and inferred abstract
        # location for each module
        p = []
        # Use the same transformation for each frequency module: leaky relu for
        # sparsity
        for f in range(self.hyper["n_f"]):
    ...
    def x2x_(self, x):
        ...
        # frequency module separately: matrix multiplication by W_tile prepares x for
        # outer product with g by element-wise multiplication
        x_ = [
    ...
    def f_mu_g_path(self, a_prev, g_prev, no_direc=None):
        ...
        # omitted (e.g. no shiny objects, or in inference model: set to all false
        ...
        # action, so no step needs to be calculated for those
        ...
        if self.hyper["has_static_action"]:
            ...
            # appears, the action vector should be all zeros. All other actions should
            # have a 1 in the label-1 entry
        ...
        # environments where transition direction needs to be omitted (can set only if
        # any no_direc)
        for f in range(self.hyper["n_f"]):
            D_a[f][no_direc, :] = self.D_no_a[f]
        # Reshape transition weight vector into transition matrix. The number of rows
        # in the transition matrix is given by the incoming abstract location
        # connections for each frequency module
        D_a = [
            torch.reshape(
                D_a[f_to],
                (
        ...
        # connected to each frequency module
        g_in = [
            torch.unsqueeze(
                torch.cat(
                    [
        ...
        # Not in the paper, but recommended by James for stability: use inferred code
        # as *difference* in abstract location. Calculate new abstract location from
        # previous abstract location and difference
        g_step = [
        ...
        # from prior on abstract location if there wasn't
        return [
            torch.stack(
                [
    ...
    def f_sigma_g_path(self, a_prev, g_prev):
        ...
        a_do_step = [a != None for a in a_prev]
        # Multi layer perceptron to generate standard deviation from all previous
        # abstract locations, including those that were just initialised and not real
        # previous locations
        from_g = self.MLP_sigma_g_path(g_prev)
        # And take exponent to get prior sigma for the walks that didn't have a
        # previous location
        ...
        # location if there was one, and the prior standard deviation on abstract
        # location otherwise
        return [
            torch.stack(
                [
    ...
    def f_mu_g_mem(self, g_downsampled):
        # Multi layer perceptron to generate mean of abstract location from
        # down-sampled abstract location, obtained by summing over sensory dimension of
        # grounded location
        return self.MLP_mu_g_mem(g_downsampled)

    def f_sigma_g_mem(self, g_downsampled):
        # Multi layer perceptron to generate standard deviation of abstract location
    ...
    def f_mu_g_shiny(self, shiny):
        # Multi layer perceptron to generate mean of abstract location from boolean
        # location shiny-ness
        ...
        # Then apply clamp and leaky relu to get object vector module activations,
        # like it's done for ground location activations
        g = self.f_p(mu_g)
        return g

    def f_sigma_g_shiny(self, shiny):
        # Multi layer perceptron to generate standard deviation of abstract location
        # from boolean location shiny-ness
        return self.MLP_sigma_g_shiny(shiny)

    def f_sigma_p(self, p):
        # Multi layer perceptron to generate standard deviation of grounded location
        # retrieval
    ...
    def f_x(self, p):
        ...
        # ground location
        # p has dimensions n_p[0]. We'll need to transform those to temporally filtered
        # sensory experience, before we can decompress. p is the flattened (by
        # concatenating rows - like reading sentences) outer product of g and x
        # (p = g^T * x). Therefore, to get the sensory experience x for a grounded
        # location p, sum over all abstract locations g for each component of x. That's
        # what the paper means when it says "sum over entorhinal preferences". It can
        # be done with the transpose of W_tile
    ...
    def f_p(self, p):
        # Calculate activation for inferred grounded location, using a leaky relu for
        # sparsity. Either apply to full multi-frequency grounded location or single
        # frequency module
        activation = (
    ...
    def attractor(self, p_query, M, retrieve_it_mask=None):
        # Retreive grounded location from attractor network memory with weights M by
        # pattern-completing query. For example, initial attractor input can come from
        # abstract location (g_) or sensory experience (x_). Start by flattening query
        # grounded locations across frequency modules
        ...
        # retrieval for low frequencies, using a mask. If not specified: initialise
        # mask as all 1s
        retrieve_it_mask = (
        ...
        for tau in range(self.hyper["i_attractor"]):
            ...
            # the mask. NB retrieve_it_mask entries have only one row, but are
            # broadcasted to batch_size
        ...
        # memory retrieval turned it into one long vector
    ...
    def hebbian(
        ...
        # product of learned vectors. p_inferred corresponds to p in the paper, and
        # p_generated corresponds to p^. The order of p + p^ and p - p^ is reversed
        # since these are row vectors, instead of column vectors in the paper.
        M_new = torch.squeeze(
            torch.matmul(
...
class MLP(torch.nn.Module):
    def __init__(
        ...
        # lists? If not, make them (but remember it wasn't)
    ...
    def set_weights(self, from_layer, value):
        ...
        # specified value
        with torch.no_grad():
            # MLP is set up as follows: w[module][layer] is Linear object,
            # w[module][layer].weight is Parameter object for linear weights,
            # w[module][layer].weight.data is tensor of weight values
            for n in range(self.N):
    ...
    def forward(self, data):
        ...
        for n in range(self.N):
            ...
            # Transpose output again to go back to column vectors instead of row
            # vectors
            output.append(module_output)
        # If this network doesn't consist of modules: select output from first module
        # to return
        if not self.is_list:
            output = output[0]
        # And return output
...
class LSTM(torch.nn.Module):
    ...
    def forward(self, data, prev_hidden=None):
        ...
        # Apply linear network to lstm output to get output: prediction at each
        # timestep
