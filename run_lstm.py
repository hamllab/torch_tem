...
# Own module imports
from tem import world, parameters, model
...
lstm = model.LSTM(
    params["n_x"] + params["n_actions"], 100, params["n_x"], n_a=params["n_actions"]
)

# Create set of training worlds, as many as there are batches
environments = [
    world.World("./graphs/5x5.json", 45) for batch in range(params["n_batches"])
]

# Create walks on each world
walks = [
    env.generate_walks(params["walk_length"], params["n_walks"]) for env in environments
]

# Create batched walks: instead of having walks separated by environment, collect them by environment
batches = [
    [[[], [], []] for l in range(params["walk_length"])]
    for w in range(params["n_walks"])
]
for env in walks:
...
for i, walk in enumerate(batches):
    ...
    chunks = [
        [i, min(i + params["n_rollout"], len(walk))]
        for i in range(0, len(walk), params["n_rollout"])
    ]
    ...
    for j, [start, stop] in enumerate(chunks):
        ...
        accuracy = torch.mean(
            (
                torch.argmax(data[:, 1:, : params["n_x"]], dim=-1)
                == torch.argmax(predictions[:, :-1, :], dim=-1)
            ).type(torch.float)
        ).numpy()
        # Show progress
        if j % 10 == 0:
            print(
                ...
                + "Loss: {:.2f}, accuracy: {:.2f} %".format(
                    loss.detach().numpy(), accuracy * 100.0
                )
            )
